...
function ColumnConfig({ columns, onSave, onCancel, showSpecialColumn = true }) {
  ...
  useEffect(() => {
    let normals = columns.filter(c => !c.is_special).map(col => ({
      ...
      type: col.column_type || 'numeric',
      maxValue: col.max_value || 10,
      weight: col.weight || 0,
      required: col.is_required || false,
      is_special: false
    }));
    if (showSpecialColumn) {
      const existing = columns.find(c => c.is_special === true);
      if (existing) {
        setLocalColumns([
          {
            name: existing.column_name || 'Promedio de Parciales',
            type: 'numeric',
            maxValue: existing.max_value || 10,
  ...
  const addColumn = () => setLocalColumns([...localColumns, { name: '', type: 'numeric', maxValue: 10, weight: 0, required: false, is_special: false }]);

  const updateColumn = (idx, field, val) => {
    const updated = [...localColumns];
    if (updated[idx].is_special && field !== 'weight' && field !== 'required') return;
  ...
  const removeColumn = (idx) => {
    if (localColumns[idx].is_special) { alert('No se puede eliminar la columna de promedio de parciales'); return; }
    if (localColumns.filter(c => !c.is_special).length === 1) { alert('Debe haber al menos una columna adicional'); return; }
    setLocalColumns(localColumns.filter((_, i) => i !== idx));
  };

  const handleSave = () => {
    const normals = localColumns.filter(c => !c.is_special);
    if (!normals.every(c => c.name && c.name.trim())) { alert('Todas las columnas deben tener un nombre'); return; }
    const totalWeight = localColumns.filter(c => c.type === 'numeric').reduce((s, c) => s + (parseFloat(c.weight) || 0), 0);
    if (totalWeight > 100) { alert(`La suma de pesos es ${totalWeight.toFixed(1)}%. No puede superar 100%`); return; }
    ...
    const validated = localColumns.map(c => ({
      name: c.name.trim(),
      type: c.type || 'numeric',
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full max-h-[95vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="mb-6 bg-gray-50 border-2 border-gray-200 rounded-lg p-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className={`text-2xl font-bold ${totalWeight > 100 ? 'text-red-600' : totalWeight === 100 ? 'text-green-600' : 'text-yellow-600'}`}>{totalWeight.toFixed(1)}%</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
              <div className={`h-full transition-all ${totalWeight > 100 ? 'bg-red-500' : totalWeight === 100 ? 'bg-green-500' : 'bg-yellow-500'}`} style={{ width: `${Math.min(totalWeight, 100)}%` }} />
            </div>
          </div>
              ...
              {localColumns.map((col, idx) => (
                <div key={idx} className={`bg-white border-2 rounded-lg p-4 shadow-sm ${col.is_special ? 'border-blue-400 bg-blue-50' : 'border-gray-200 hover:border-blue-300'}`}>
                  <div className="flex items-center gap-3 mb-3">
                    <div className={`font-bold rounded-full w-8 h-8 flex items-center justify-center text-sm ${col.is_special ? 'bg-blue-600 text-white' : 'bg-blue-100 text-blue-700'}`}>{idx + 1}</div>
                    <h3 className="font-semibold text-gray-700">{col.name || `Columna ${idx + 1}`}{col.is_special && <span className="ml-2 text-xs bg-blue-200 text-blue-800 px-2 py-0.5 rounded">Fija (Promedio Parciales)</span>}</h3>
                  ...
                  <div className="grid grid-cols-12 gap-3">
                    <div className="col-span-12 md:col-span-4">
                      ...
                      <input type="text" value={col.name} onChange={e => updateColumn(idx, 'name', e.target.value)} disabled={col.is_special} className={`w-full border-2 rounded-lg px-3 py-2 focus:border-blue-500 outline-none ${col.is_special ? 'bg-gray-100' : ''}`} placeholder={col.is_special ? 'Promedio de Parciales' : 'Ej: Tarea 1, Examen Final'} />
                    </div>
                    ...
                    <div className="col-span-6 md:col-span-1 flex items-end">
                      <button onClick={() => removeColumn(idx)} disabled={col.is_special} className={`w-full px-3 py-2 rounded-lg font-semibold ${col.is_special ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-red-500 text-white hover:bg-red-600'}`}>🗑️</button>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <button onClick={addColumn} className="mt-4 w-full border-3 border-dashed border-blue-400 bg-blue-50 text-blue-700 py-4 rounded-lg font-bold text-lg hover:bg-blue-100">+ Agregar Nueva Columna</button>
          <div className="mt-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-500 rounded-lg p-4">
            ...
            <ul className="space-y-1 text-sm text-blue-800">
              {showSpecialColumn && <li>• <strong>Promedio de Parciales (fijo):</strong> Se calcula automáticamente como el promedio de las calificaciones finales de los tres parciales. Puedes asignarle un peso.</li>}
        ...
        <div className="border-t-2 bg-gray-50 p-6">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <div><div className="text-sm text-gray-600">{localColumns.length} columnas</div><div className={`text-lg font-bold ${totalWeight > 100 ? 'text-red-600' : totalWeight === 100 ? 'text-green-600' : 'text-yellow-600'}`}>Total: {totalWeight.toFixed(1)}% / 100%</div></div>
            <div className="flex gap-3"><button onClick={onCancel} className="px-6 py-3 border-2 border-gray-300 rounded-lg hover:bg-gray-100 font-semibold">Cancelar</button><button onClick={handleSave} className="px-8 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 font-bold shadow-lg">Guardar Configuración</button></div>
          </div>
        </div>
      </div>
    </div>
